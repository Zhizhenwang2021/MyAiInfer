...
TEST(test_buffer, allocate) {
  ...
  using namespace base;
  auto alloca = CPUDeviceAllocatorFactory::get_instance();
  Buffer buffer(32, alloca);
  ASSERT_NE(buffer.ptr(), nullptr);
}
...
TEST(test_buffer, use_external) {
   ...
   * 2.alloca定义
   * 3.定义外部指针
   * 4.定义buffer 传入alloca和指针
   * 5.判断buffer.is_external是否为true
   * 6.删除外部指针
   */
  using namespace base;
  auto alloca = CPUDeviceAllocatorFactory::get_instance();
  float* ptr = new float[32];
  Buffer buffer(32, alloca, ptr, true);
  ASSERT_EQ(buffer.is_external(), true);
  delete[] ptr;
}
