...
Tensor::Tensor(base::DataType data_type, int32_t dim0, bool need_alloc,
    std::shared_ptr<base::DeviceAllocator> alloc, void *ptr) : data_type_(data_type) {
    dims_.push_back(dim0);
    size_ = dim0;
    if (need_alloc && alloc != nullptr) {
        allocate(alloc);
    } else {
        if (ptr != nullptr) {
...
void Tensor::reshape(const std::vector<int32_t>& dims) { //细看下
